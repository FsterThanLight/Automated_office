...
def commond(root,setting):
    '''在屏幕上创建两个按钮'''
    button_1_2=[]
    name=['输出术语(f1)','导出文件(f2)']
    for i in range(len(name)):
        button_1=creat_commond(root,name[i],setting.place_button[0],\
...
def rrr(number):
    ...
    if number==1:
        ...
        wb_2.save('特长隧道、特大桥梁及梁板日报表-宁攀高速zcb1-14、19项目部'+\
        date_new_name+'.xlsx')
        #wb_2.close()
        #app.quit()
...
累计完成C20砼面板浇筑'+str(dpvr[6])+'平方。\n\n'+text_list_5[1])
    ...
    #text_all[3].insert(END,'1#梁场今日完成:\n\n2#梁厂今日完成:')
