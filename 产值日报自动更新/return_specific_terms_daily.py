...
def return_specific_terms(cursor,new_date,x,judge):
    '''将信息组合为特定术语'''
    # cursor.execute('SELECT 桥梁名称,梁片,梁片编码 \
    # FROM 所有已浇筑梁片信息 WHERE 浇筑日期=#2021/12/9#;')
    if judge=='浇筑':
        cursor.execute("SELECT 桥梁名称,梁片,梁片编码 \
        FROM 所有已浇筑梁片信息 WHERE 浇筑日期=#"+new_date+"# AND 浇筑单位='"+x+"';")
    elif judge=='安装':
        cursor.execute("SELECT 桥梁名称,梁片,梁片编码 \
        FROM 所有已浇筑梁片信息 WHERE 安装日期=#"+new_date+"# AND 安装单位='"+x+"';")
...
def return_specific_terms_qmx(cursor,new_date,x,judge):
    '''将信息组合为特定术语桥面系相关'''
    # if judge=='湿接缝名称':
    ...
    if judge=='湿接缝名称':
        cursor.execute("SELECT 桥梁名称,跨 \
        FROM 已完成湿接缝汇总查询 WHERE 完成日期=#"+new_date+"# AND 完成单位='"+x+"';")
    elif judge=='防撞护栏名称':
        cursor.execute("SELECT 桥梁名称,联 \
        FROM 已完成防撞护栏汇总查询分左右侧 WHERE 完成日期=#"+new_date+"# AND 完成单位='"+x+"';")
    elif judge=='桥面铺装名称':
        cursor.execute("SELECT 桥梁名称,联 \
        FROM 已完成桥面铺装查询 WHERE 完成日期=#"+new_date+"# AND 完成单位='"+x+"';")
    beam_info_1=cursor.fetchall()
    beam_info_1.sort()
